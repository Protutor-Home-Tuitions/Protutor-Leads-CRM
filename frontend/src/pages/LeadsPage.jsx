...
function CopyMapLink({ url }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
      <a href={url} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', textDecoration: 'underline', fontSize: '12px' }}>Map</a>
      <button type="button" onClick={handleCopy}
        style={{ cursor: 'pointer', padding: '2px 6px', borderRadius: '4px', background: copied ? '#d1fae5' : '#ede9fe', border: 'none', fontSize: '10px', fontWeight: 600, color: copied ? '#16a34a' : '#7c3aed', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
        {copied ? '✓ Copied!' : '📋 Copy'}
      </button>
    </span>
  );
                        ...
                        <td colSpan={5} style={{ padding: '0 20px 14px' }}>
                          <div style={{ background: '#faf5ff', border: '1px solid #e9d5ff', borderRadius: '8px', padding: '8px 0', marginLeft: '82px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '70px 70px 150px 140px 1fr 110px 120px 100px', alignItems: 'center', fontSize: '12px', color: '#5b21b6' }}>
                              ...
                              <span style={{ padding: '0 8px', borderLeft: '1px solid #e9d5ff' }}>{'Quote: '}<span style={{ fontWeight: 600, color: lead.quoteAccepted === 'Yes' || lead.quoteAccepted === true ? '#16a34a' : lead.quoteAccepted ? '#dc2626' : '#a78bfa' }}>{lead.quoteAccepted ? String(lead.quoteAccepted) : '—'}</span></span>
                              <span style={{ padding: '0 8px', borderLeft: '1px solid #e9d5ff' }}>{lead.expectedQuote ? (
                                <span style={ quoteRejected ? { background: '#fee2e2', border: '1.5px solid #dc2626', borderRadius: '4px', padding: '1px 6px', color: '#991b1b', fontWeight: 700 } : {}}>
                                  {'Parent: '}<strong>{lead.expectedQuote}</strong>
                                </span>
                              ) : '—'}</span>
